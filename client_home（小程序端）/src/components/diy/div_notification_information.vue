<template>
	<view class="diy_details diy_div_notification_information">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','notification_date')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>通知日期:</span>
						</view>
							<view class="diy_field diy_date">
							<text>
								{{ $toTime(obj["notification_date"],"yyyy-MM-dd") }}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','notification_content')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>通知内容:</span>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/notification_information/get_obj", {
					notification_information_id:this.query.notification_information_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["notification_information_id"] !==0) {
...
<style scoped>
	.div_notification_information_details {}
