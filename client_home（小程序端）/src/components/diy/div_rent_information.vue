<template>
	<view class="diy_details diy_div_rent_information">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','rent_collection_date')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>收租日期:</span>
						</view>
							<view class="diy_field diy_date">
							<text>
								{{ $toTime(obj["rent_collection_date"],"yyyy-MM-dd") }}
							</text>
						</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/rent_information/get_obj", {
					rent_information_id:this.query.rent_information_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["rent_information_id"] !==0) {
