<template>
	<view class="diy_details diy_div_housing_consultation">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','consultation_date')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>咨询日期:</span>
						</view>
							<view class="diy_field diy_date">
							<text>
								{{ $toTime(obj["consultation_date"],"yyyy-MM-dd") }}
							</text>
						</view>
						</view>
					...
					<view v-if="$check_field('get','consultation_content')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>咨询内容:</span>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/housing_consultation/get_obj", {
					housing_consultation_id:this.query.housing_consultation_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["housing_consultation_id"] !==0) {
...
<style scoped>
	.div_housing_consultation_details {}
