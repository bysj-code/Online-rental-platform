<template>
	<view class="diy_details diy_div_lease_application">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','application_description')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>申请描述:</span>
						</view>
							<view class="diy_field diy_desc">
							<text>
								{{ obj["application_description"] }}
							</text>
						</view>
						</view>
				</view>
			...
			<view class="container">
				<view class="row">
					<view class="col">
						<button type="button" class="diy_btn" @click="to_form('/pages/check_in_information/edit')" v-if="$check_action('/check_in_information/edit','add') || $check_action('/check_in_information/edit','set')" >同意入住</button>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/lease_application/get_obj", {
					lease_application_id:this.query.lease_application_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["lease_application_id"] !==0) {
