<template>
  <view class="page_lease_application diy_table diy_table_tml" id="lease_application_table">
    <!-- 筛选模块(开始) -->
    <view>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/lease_application/view?" v-show="user_group == '管理员' || $check_action('/lease_application/table','add') || $check_action('/lease_application/view','add')">添加</navigator>
                            ...
                            <span>房屋类型</span>
								...
								<text>
									{{ o["house_type"] }}
													...
													<view v-if="1 && $check_field('get','deposit')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>押金</span>
                          </view>
								...
								<text>
									{{ o['deposit'] }}
													...
													<view v-if="0 && $check_field('get','rent')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>租金</span>
                          </view>
                            ...
                            <span>租客用户</span>
								...
								<text>
									{{ get_user_tenant_user(o['tenant_user']) }}
													...
													<view v-if="0 && $check_field('get','application_description')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>申请描述</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/lease_application/table','del') || $check_action('/lease_application/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/lease_application/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/lease_application/table','set') || $check_action('/lease_application/view','set') || $check_action('/lease_application/view','get')" class="btn btn-primary">详情</navigator>
	                    <a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/lease_application/table')" class='btn btn-primary'>支付</a>
	                                <!--跨表按钮-->
                                <a type="button" size="mini" class="btn btn-primary" v-if="$check_action('/check_in_information/view','add')" @click="to_form('/pages/check_in_information/view',o)">
                      <span>同意入住</span>
                    </a>
                        					</view>
				</view>
          ...
          submit_pay(){
            ...
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/lease_application/table");
                }, 700);
              }else if(res.error){
