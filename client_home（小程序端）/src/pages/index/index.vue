<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="房源信息推荐"
      url="/pages/housing_information/list"
      :list="list_article"
      v-if="$check_action('/housing_information/list', 'get')"
    >
	  <list_housing_information :list="list_housing_information"></list_housing_information>
		...
		*/
		get_housing_information() {
			...
			this.$get(url, {
					...
					console.log("房源信息" ,json.result.list);
					this.list_housing_information = json.result.list;
											this.list_housing_information.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_housing_information ,"housing_information" ,"13472");
									}
			});
		},
