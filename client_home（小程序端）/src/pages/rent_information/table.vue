<template>
  <view class="page_rent_information diy_table diy_table_tml" id="rent_information_table">
    <!-- 筛选模块(开始) -->
    <view>
              ...
              <uni-forms :modelValue="query">
                    <uni-forms-item label="房屋名称" name="house_name">
                      <uni-easyinput type="text" v-model="query.house_name" placeholder="房屋名称" />
                                  ...
                                  <uni-forms-item label="收租日期" name="rent_collection_date">
                      <uni-easyinput type="text" v-model="query.rent_collection_date" placeholder="收租日期" />
                    </uni-forms-item>
                  </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/rent_information/view?" v-show="user_group == '管理员' || $check_action('/rent_information/table','add') || $check_action('/rent_information/view','add')">添加</navigator>
                            ...
                            <span>房主用户</span>
								...
								<text>
									{{ get_user_homeowner_user(o['homeowner_user']) }}
													...
													<view v-if="0 && $check_field('get','name_of_the_owner')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>房主姓名</span>
                          </view>
                            ...
                            <span>租金</span>
								...
								<text>
									{{ o['rent'] }}
													...
													<view v-if="1 && $check_field('get','rent_collection_date')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>收租日期</span>
                          </view>
								...
								<text>
									{{ $toTime(o["rent_collection_date"],"yyyy-MM-dd") }}
								</text>
							</view>
										</view>
					</view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/rent_information/table','del') || $check_action('/rent_information/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/rent_information/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/rent_information/table','set') || $check_action('/rent_information/view','set') || $check_action('/rent_information/view','get')" class="btn btn-primary">详情</navigator>
	                    <a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/rent_information/table')" class='btn btn-primary'>支付</a>
	    					</view>
				</view>
			</view>
		</view>
          ...
          submit_pay(){
            ...
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/rent_information/table");
                }, 700);
              }else if(res.error){
