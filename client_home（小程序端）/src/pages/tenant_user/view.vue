        ...
        <view>
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','tenant_name') || $check_field('add','tenant_name') || $check_field('set','tenant_name')" label="租客姓名" name="tenant_name">
                                <uni-easyinput type="text" v-model="form['tenant_name']" v-if="user_group === '管理员' || (form['tenant_user_id'] && $check_field('set','tenant_name')) || (!form['tenant_user_id'] && $check_field('add','tenant_name'))" :disabled="disabledObj['tenant_name_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','tenant_name')">
                  {{ form['tenant_name'] }}
                </text>
                            </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" label="性别" name="gender">
                  ...
                  v-if="user_group === '管理员' || (form['tenant_user_id'] && $check_field('set','gender')) || (!form['tenant_user_id'] && $check_field('add','gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      url_add: "~/api/tenant_user/add?",
      url_set: "~/api/tenant_user/set?",
      url_get_obj: "~/api/tenant_user/get_obj?",
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/tenant_user/upload?'),
