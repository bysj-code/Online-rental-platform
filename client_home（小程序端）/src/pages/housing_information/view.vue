          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','house_no') || $check_field('add','house_no') || $check_field('set','house_no')" label="房屋编号" name="house_no">
                                <uni-easyinput type="text" v-model="form['house_no']" v-if="user_group === '管理员' || (form['housing_information_id'] && $check_field('set','house_no')) || (!form['housing_information_id'] && $check_field('add','house_no'))"  :disabled="true" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','house_name') || $check_field('add','house_name') || $check_field('set','house_name')" label="房屋名称" name="house_name">
                                <uni-easyinput type="text" v-model="form['house_name']" v-if="user_group === '管理员' || (form['housing_information_id'] && $check_field('set','house_name')) || (!form['housing_information_id'] && $check_field('add','house_name'))" :disabled="disabledObj['house_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','house_type') || $check_field('add','house_type') || $check_field('set','house_type')" label="房屋类型" name="house_type">
                  ...
                  :clear="!disabledObj['house_type_isDisabled']"
                  :disabled="disabledObj['house_type_isDisabled']"
              ...
              <uni-forms-item v-if="$check_field('get','deposit') || $check_field('add','deposit') || $check_field('set','deposit')" label="押金" name="deposit">
                                <uni-easyinput type="text" v-model="form['deposit']" v-if="user_group === '管理员' || (form['housing_information_id'] && $check_field('set','deposit')) || (!form['housing_information_id'] && $check_field('add','deposit'))" :disabled="disabledObj['deposit_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','rent') || $check_field('add','rent') || $check_field('set','rent')" label="租金" name="rent">
                                <uni-easyinput type="text" v-model="form['rent']" v-if="user_group === '管理员' || (form['housing_information_id'] && $check_field('set','rent')) || (!form['housing_information_id'] && $check_field('add','rent'))" :disabled="disabledObj['rent_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','hall') || $check_field('add','hall') || $check_field('set','hall')" label="厅室" name="hall">
                  ...
                  v-model="form.hall"
                  :localdata="list_hall"
                  :clear="!disabledObj['hall_isDisabled']"
                  :disabled="disabledObj['hall_isDisabled']"
              ...
              <uni-forms-item v-if="$check_field('get','detailed_address') || $check_field('add','detailed_address') || $check_field('set','detailed_address')" label="详细地址" name="detailed_address">
                                <uni-easyinput type="text" v-model="form['detailed_address']" v-if="user_group === '管理员' || (form['housing_information_id'] && $check_field('set','detailed_address')) || (!form['housing_information_id'] && $check_field('add','detailed_address'))" :disabled="disabledObj['detailed_address_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','detailed_address')">
                  {{ form['detailed_address'] }}
                </text>
                            </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','house_details') || $check_field('add','house_details') || $check_field('set','house_details')" label="房屋详情" name="house_details">
                      </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','homeowner_user') || $check_field('add','homeowner_user') || $check_field('set','homeowner_user')" label="房主用户" name="homeowner_user">
                  ...
                  v-if="user_group === '管理员' || (form['housing_information_id'] && $check_field('set','homeowner_user')) || (!form['housing_information_id'] && $check_field('add','homeowner_user'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['homeowner_user']"
                  :localdata="list_user_homeowner_user"
              ...
              <uni-forms-item v-if="$check_field('get','name_of_the_owner') || $check_field('add','name_of_the_owner') || $check_field('set','name_of_the_owner')" label="房主姓名" name="name_of_the_owner">
                                <uni-easyinput type="text" v-model="form['name_of_the_owner']" v-if="user_group === '管理员' || (form['housing_information_id'] && $check_field('set','name_of_the_owner')) || (!form['housing_information_id'] && $check_field('add','name_of_the_owner'))" :disabled="disabledObj['name_of_the_owner_isDisabled']" />
                <!-- 仅查看 -->
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/housing_information/upload?'),
    ...
    async get_group_user_homeowner_user() {
      ...
      if(json.result && json.result.obj){
        ...
        this.get_user_session_homeowner_user(this.form['homeowner_user'])
      }
      else if(json.error){
        console.error(json.error);
      }
