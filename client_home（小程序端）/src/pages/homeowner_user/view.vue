          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','name_of_the_owner') || $check_field('add','name_of_the_owner') || $check_field('set','name_of_the_owner')" label="房主姓名" name="name_of_the_owner">
                                <uni-easyinput type="text" v-model="form['name_of_the_owner']" v-if="user_group === '管理员' || (form['homeowner_user_id'] && $check_field('set','name_of_the_owner')) || (!form['homeowner_user_id'] && $check_field('add','name_of_the_owner'))" :disabled="disabledObj['name_of_the_owner_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" label="性别" name="gender">
                  ...
                  v-if="user_group === '管理员' || (form['homeowner_user_id'] && $check_field('set','gender')) || (!form['homeowner_user_id'] && $check_field('add','gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "homeowner_user",
      is_password: true,
      field: "homeowner_user_id",
      url_add: "~/api/homeowner_user/add?",
      url_set: "~/api/homeowner_user/set?",
      url_get_obj: "~/api/homeowner_user/get_obj?",
      url_upload: "~/api/homeowner_user/upload?",
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/homeowner_user/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["homeowner_user_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?homeowner_user_id=" + form_sub["homeowner_user_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/homeowner_user/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/homeowner_user/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/homeowner_user/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/homeowner_user/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/homeowner_user/view','get');
